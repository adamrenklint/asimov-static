...
module.exports = Initializer.extend({
  ...
  'postRender': function (page) {
    ...
    //   rendered = rendered.replace('</head>', '<script>document.write(\'<script src="http://\' + (location.host || \'localhost\').split(\':\')[0] + \':' + asimov.config('server.liveReloadPort') + '/livereload.js"></\' + \'script>\')</script></head>');
    // }
