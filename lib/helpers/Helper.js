...
module.exports = Base.extend({
  ...
  'initialize': function () {
    ...
    var lowercased = self.options.name.toLowerCase();
    if (self.options.name !== lowercased) {
      handlebars.registerHelper(lowercased, function () {
        if (warned.indexOf(lowercased) === -1) {
          self.logger.pending(self.namespace, 'Using deprecated lowercased helper name "' + lowercased + '" for "' + self.options.name + '"', 'red');
