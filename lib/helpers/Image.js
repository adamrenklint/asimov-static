...
module.exports = Helper.extend({
  ...
  'run': function (src, params) {
    ...
    if (params.asBackgroundImage) {
      self.assert('string', params.width, 'You must include a width attribute to use the asBackgroundImage flag');
      self.assert('string', params.height, 'You must include a height attribute to use the asBackgroundImage flag');
      params['data-url'] = params.src;
      tag = 'div';
      params.style = 'background-image:url(' + params.src + '); width:' + params.width + 'px; height:' + params.height + 'px;';
