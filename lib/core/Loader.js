...
module.exports = Base.extend({
  ...
  'initialize': function () {
    ...
    var message = 'Running in ' + process.env.ENV.toUpperCase() + ' environment';
    if (process.env.USE_STATIC_PUBLIC) {
      message += ', serving prebuilt static content';
    }
    if (process.env.PREBUILD_STATIC_PUBLIC) {
      message += ', prebuilding static content';
    }
  ...
  'bootstrap': function () {
    ...
    self.runSequential('initializer', self.options.paths).done(function (count) {
      ...
      self.bindOnceTo(self.mediator, 'queue:empty', function () {
        ...
        if (process.env.PREBUILD_STATIC_PUBLIC) {
          self.logger.since(self.namespace, 'Static prebuild complete', self.started);
      ...
      if (process.env.USE_STATIC_PUBLIC) {
        return asimov.config('server.ready', true);
      }
