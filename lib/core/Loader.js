...
module.exports = Base.extend({
  ...
  'bootstrap': function () {
    ...
    self.runSequential('initializer', self.options.paths).done(function (count) {
      ...
      // self.logger.since('asimov-static', 'Executed ' + count + ' website initializer(s)', self.started);

      self.bindOnceTo(self.mediator, 'queue:empty', function () {
        console.log('EMPTY');
      });

        // self.logger.since('asimov-static', 'Started website "' + asimov.config('PKG').name + '" @ http://127.0.0.1:' + asimov.config('server.port'), self.started);
