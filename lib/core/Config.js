...
module.exports = Base.extend({
  ...
  'renderValues': function (hash) {
    ...
    var self = this;
    var asimovPath = npath.join(__dirname, '../..');
  ...
  'decorateInitializers': function (config) {
    ...
    var frameworkInitializers = ['Collections'];
    if (!process.env.USE_STATIC_PUBLIC) {
      frameworkInitializers = frameworkInitializers.concat([
        'CollectionEvents',
    ...
    frameworkInitializers.push('Sitemap');
  ...
  'getPath': function (path) {
    ...
    var containsAsimovPath = path.indexOf(asimovPath) >= 0;
  ...
  'generate': function (environment) {
    ...
    var frameworkConfigPath = npath.join(__dirname, '../config.json');
