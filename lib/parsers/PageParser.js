...
module.exports = DependencyParser.extend({
  ...
  'initialize': function () {
    ...
    self.bindTo(self.mediator, 'register', 'register');
  },

  'register': function (model, dependency) {

    var self = this;
    var path = typeof dependency === 'string' ? dependency : dependency.attributes.path;
    self.add(model, path, self.options.watcher);
  },
  ...
  'parse': function (model, raw, dependencies) {
    ...
    var templatePathRE = new RegExp('/' + attributes.template + '\.tmpl$');
    var template = self.options.templates.find(function (tmpl) {
      return tmpl.attributes.path.match(templatePathRE);
    });
